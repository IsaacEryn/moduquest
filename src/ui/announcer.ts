...
export class Announcer {
  ...
  private say(el: HTMLElement, text: string): void {
    // 같은 문구가 연속돼도 다시 낭독되도록, 반복될 때는 보이지 않는 문자를 붙여
    // 항상 내용이 달라지게 만든다
    const zwsp = '\u200B'
    const prev = (el.textContent ?? '').replace(zwsp, '')
    el.textContent = prev === text ? text + zwsp : text
    this.onLine?.(text)
  }

  polite(text: string): void {
    this.say(this.log, text)
  }

  assertive(text: string): void {
    this.say(this.alert, text)
  }
  ...
  private handle(e: GameEvent): void {
    switch (e.type) {
      case 'moved': {
        let text = `${DIR_KO[e.dir]}으로 한 칸.`
        if (e.ahead) text += ` 앞에 ${e.ahead}.`
        this.polite(text)
        break
      }
      case 'blocked':
        this.polite('벽이다. 갈 수 없다.')
        break
      case 'fieldSummary':
      case 'battleSummary':
        this.polite(e.text)
      ...
      case 'attacked': {
        ...
        this.polite(
          `${who(e.actor)}의 공격. ${target}에게 ${e.damage} 피해.${left}`,
        )
        this.caption('[타격]')
        break
      }
      case 'healed': {
        const target = e.target.isPlayer ? '내' : josa(e.target.name, '이', '가')
        this.polite(
      ...
      case 'taunted':
        this.polite(
          `${e.actor.name}의 도발. ${e.duration}라운드 동안 적이 ${e.actor.name}를 노린다.`,
        )
        this.caption('[도발]')
        break
      case 'defended':
