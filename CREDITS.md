...
저장소에 있는 그림 파일은 `public/brand/`의 아홉 개뿐이고, 그것도 위의 스프라이트
데이터에서 뽑아낸 것이다. `tools/buildBrand.mjs`가 `src/render/sprites.ts`를 읽어
SVG와 PNG로 낸다 — 다섯이 나란히 선 것이 로고다. 손으로 그린 그림이 아니라 게임에
이미 있는 것을 다시 쓴 결과라, 게임 화면과 브랜드가 어긋날 자리가 없다.
